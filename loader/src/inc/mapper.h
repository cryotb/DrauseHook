...
#define TARGET_NAME _XS("r5apex")      // R5Apex
#define MAPPING_NAME _XS("r5apex.exe") // r5apex.exe
...
using relocation_list = std::vector<relocation_info_t>;
...
struct LoaderContext
{
    ...
    */
    ...
    u64 libc_base;
    u64 libm_base;
    string_table::String_table* strtbl;
...
namespace mapper
{
    extern relocation_list get_relocation_list(u64 image_base, PIMAGE_NT_HEADERS64 nt_header);

    extern bool resolve_linux_crt();

    extern bool setup_pre();
    extern bool setup(int pid);
    extern bool fetch_image(std::vector<u8>& buf);
    extern bool load(std::vector<u8> &buf);
}
